...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    counts=[count/numTrials for count in counts]

    pylab.plot(counts, label = "SimpleVirus")
...
#simulationWithoutDrug(100,1000,0.1,0.05,40)
# PROBLEM 3
#
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in newviruslist:
            try:
                newvirus=virus.reproduce(newDensity,self.drugs)
            except NoChildException:
                pass
            else:self.viruses.append(newvirus)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    maxPop: maximum virus population for pae(tient (an integer)
    maxBirthProb: Maximum reproduction probability (a float between 0-1)        
    clearProb: maximum clearance probability (a float between 0-1)
    ...
    for i in range(numTrials):
        ...
        for t in range(150):
            numvirus[t]+=p.update()
            numresistantvirus[t]+=p.getResistPop(['guttagonol'])
    ...
    numvirus=[num/numTrials for num in numvirus]
    numresistantvirus=[num/numTrials for num in numresistantvirus]

    pylab.plot(numvirus, label = "numvirus")
    pylab.plot(numresistantvirus, label = "numresistantvirus")
