...
def generate_models(x, y, degs):
    ...
    coefs=[]
    for deg in degs:
        coefs.append(np.array(np.polyfit(x,y,deg)))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        for i in range(len(x)):
            est.append(np.polyval(model,x[i]))
        plt.plot(x,est,label="degree="+str(len(model)-1),c='r')
        rsq=r_squared(y,est)
        plt.title("rsquared="+str(rsq))
        plt.show()              
